...
class MatMulOp(Op):
    ...
    def compute(self, node, input_vals):
        ...
        assert len(input_vals) == 2
        return np.matmul(
            input_vals[0] if not node.matmul_attr_trans_A else input_vals[0].transpose(),
            input_vals[1] if not node.matmul_attr_trans_B else input_vals[1].transpose()
        )
    ...
    def gradient(self, node, output_grad):
        ...
        """
        """Done: Your code here"""
        dA = matmul_op(output_grad, node.inputs[1], False, True)
        dB = matmul_op(node.inputs[0], output_grad, True, False) 
        return [dA, dB]
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        # Sum  the partial adjoints of the output edges
        output_grad = sum_node_list(node_to_output_grads_list[node])
        node_to_output_grad[node] = output_grad
        # Gradient of the node for derivatives with respect to node.inputs
